...
const StickerScreen = (props) => {
  const arrList = sticker_base[props.festival];
  const openShare = () => {
    interstitial.load();
    var identifier = day_title[props.festival];
    var identifierFolder = identifier.split(" ").join("").toLowerCase();
      ...
      <View style={[s.mdtp20, s.pdlt10]}>
        <View style={s.row}>
          <View style={[s.fl1, s.jCenter, s.mdtp10]}>
            <Text style={[{ fontSize: 20 }, s.b]}>{day_title[props.festival]}</Text>
